...
- [ ] 2. `feat(domain)`: доменные сущности и порты — `Document`, `Chunk`, `ChatMessage`; интерфейсы `DocumentLoader`, `Chunker`, `EmbeddingProvider`, `VectorStoreRepository`, `LLMProvider`, `ChatRepository`.
- [ ] 3. `feat(infra)`: `docker-compose.yml` — Qdrant, Postgres, Redis.
...
- [ ] 19. `test`: unit + integration тесты (`pytest`) на ключевые сценарии.
