# RAG-ассистент ДагГАУ

Retrieval-Augmented Generation система для ответов по документам
университета: гибридный поиск, реранкинг, выбор LLM-модели, чаты
с персистентностью и мультипользовательский доступ.

Начинали с учебного прототипа (Streamlit + ChromaDB + сплит по `##`),
пришли к расширяемой hexagonal-архитектуре. Легаси-прототип сохранён
в `frontend-legacy/`.
...
- **Qdrant вместо ChromaDB**: именованные dense+sparse векторы и RRF
...
- **Docling вместо regex-сплита**: структура, таблицы, OCR и десятки
  форматов вместо маркера `##`.
